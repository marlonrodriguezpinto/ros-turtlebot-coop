...
class getObjekt(smach.State):
    ...
    def execute(self, userdata):
        ...
        if objekt:
            ...
            rospy.set_param('CurrentObjektR0', objekt)
    ...
    def execute(self,userdata):
        ...
        while True:
            ...
            helpcall = rospy.get_param('/Helpcall')
                ...
                return 'succeeded'
            elif rospy.has_param('/aruco_marker/result'):
                rospy.get_param
...
class searchObjekt(smach.State):
    ...
    def execute(self,userdata):
        ...
        while rospy.Duration(50) or not smach.is_shutdown():
            rospy.loginfo('ArUco searching for objekt while driving to Location')
            # moveToObjekt
            while rospy.Duration(2):
                if rospy.has_param('/NewObjektR0/distance')
                    distance = rospy.get_param('/NewObjektR0/distance')
                    angle = rospy.get_param('/NewObjektR0/rot')
                    break 

            robotToGoal.main(angle, distance)
            # stops till movement has finished

            if rospy.has_param('/aruco/result'):
                newPosition = rospy.get_param('/aruco/result')
            # moveToObjekt
            if rospy.has_param('/aruco/result'):
                newPosition = rospy.get_param('/aruco/result')
            # turnToObjekt
