...
class searchObjekt(smach.State):
    ...
    def execute(self,userdata):
        ...
        while rospy.Duration(30) or not smach.is_shutdown()):
            rospy.set_param("Current_Objekt", userdata.objekt)
            rospy.loginfo('ArUco searching for objekt while driving to Location')
            # distance = sqrt(pow((objekt.x - self.pose.x), 2) + pow((objekt.y - self.pose.y), 2))
            # angle = (atan2(objekt.y - self.pose.y, objekt.x - self.pose.x) - self.pose.theta)
            rospy.Duration(1)
            if rospy.has_param("/NewObjekt/trans"):
                distance = rospy.get_param("/NewObjekt/trans")
                angle = rospy.get_param("/NewObjekt/rot")
            robotmoveto.main(angle, distance)
